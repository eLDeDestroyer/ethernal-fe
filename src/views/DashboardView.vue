<template>
  <div
    class="min-h-screen relative overflow-hidden p-4 lg:pt-24 lg:px-10 bg-linear-to-b from-blue-50 to-blue-100"
  >
    ...
    <div class="absolute inset-0 pointer-events-none">
      <div
        class="absolute -top-20 -left-20 w-72 h-72 bg-white/30 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute bottom-0 -right-28 w-80 h-80 bg-blue-300/20 rounded-full blur-3xl"
      ></div>
    </div>
    ...
    <div class="relative">
      <!-- CARD LUAR PUTIH -->
      <div
        class="bg-white rounded-3xl p-3 shadow-xl mb-6 border border-gray-200 lg:hidden"
      >
        <!-- CARD BIRU -->
        <div
          class="relative bg-linear-to-br from-blue-900 to-blue-800 text-white rounded-3xl p-6 shadow-lg overflow-hidden"
        >
          <!-- HIASAN -->
          <div class="absolute inset-0 opacity-25 pointer-events-none">
            <div
              class="absolute -top-10 -right-10 w-40 h-40 bg-white/30 rounded-full blur-3xl"
            ></div>
            <div
              class="absolute bottom-0 left-2 w-24 h-24 bg-white/20 rounded-full blur-2xl"
            ></div>
          </div>
          ...
          <div class="relative flex justify-between items-center">
            ...
            <div class="flex flex-col items-end">
              <div
                class="w-14 h-14 rounded-2xl bg-white/20 flex items-center justify-center backdrop-blur-md"
              >
      ...
      <div class="grid grid-cols-4 lg:gap-4 mb-2 lg:mb-8 lg:grid-cols-3">
        <a
          ...
          class="flex flex-col items-center p-3 lg:bg-white lg:rounded-2xl lg:shadow-md lg:border lg:border-blue-800 lg:transition-colors lg:flex-row lg:items-center lg:justify-start lg:gap-3 lg:p-4 lg:border-blue-400 lg:h-20 hover:lg:border-blue-500 lg:h-[10rem]"
        >
          <!-- Kiri: icon -->
          <div
            class="w-14 h-14 lg:w-30 lg:h-30 flex items-center justify-center bg-white lg:bg-white rounded-xl lg:shrink-0"
          >
            <img
              :src="item.img"
              alt="icon"
          ...
          <div class="mt-2 text-center lg:mt-0 lg:text-left flex-1">
            <p
              class="text-sm hidden font-semibold text-gray-700 lg:text-[1.5rem] absolute mt-[-2.5rem] lg:block"
            >
              {{ item.short_category_name }}
            </p>
            <p
              class="text-sm hidden font-semibold text-gray-700 lg:text-base lg:block"
            >
              {{ item.category_name }}
            </p>
            <p class="text-xs font-semibold lg:text-gray-400 block lg:hidden">
              {{ item.short_category_name }}
            </p>
          </div>
        </a>
      ...
      <h2 class="text-lg font-bold text-gray-800 mb-3 lg:text-[1.5rem]">
        Riwayat Latihan
      </h2>

      <div class="flex space-x-4 overflow-x-auto no-scrollbar pb-4">
        <a
          :href="`/answer/${item.uuid}`"
        ...
        >
          ...
          <h3
            class="text-[1.05rem] lg:text-[1.25rem] font-bold text-gray-900 mt-1 leading-snug"
          >
            {{ item.title }}
          </h3>
          ...
          <div class="mt-2 text-xs lg:text-sm text-gray-600 space-y-0.5">
            <p>
              <b>Nilai:</b> {{ item.total_score }} / {{ item.total_question }}
            </p>
          ...
          <div class="mt-3">
            <div class="h-1.5 lg:h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                :style="{
                  width: (item.total_score / item.total_question) * 100 + '%',
                }"
              ></div>
            </div>
    ...
    if (response.status) {
      ...
      if (data.categories && data.categories.length > 0) {
        categories.value = data.categories.map((cat) => ({
          ...cat,
          img:
            categoryImages[cat.short_category_name] ||
